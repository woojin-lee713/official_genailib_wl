...
def mock_get_chat_response(mocker: MockFixture) -> Mock:
    return mocker.patch("genailib_rg.get_chat_response", return_value=Mock(text="Test response from OpenAI"))
    def test_main_prints_response(runner: CliRunner, mock_get_chat_response: Mock) -> None:
        result = runner.invoke(main, ["--prompt", "Tell me something interesting"])
        assert "Test response from OpenAI" in result.output

def test_main_invokes_openai_api(runner: CliRunner, mock_get_chat_response: Mock) -> None:
    runner.invoke(main, ["--prompt", "Hello"])
    mock_get_chat_response.assert_called_with(prompt="Hello")

def test_main_fail_on_api_error(runner: CliRunner, mock_get_chat_response: Mock) -> None:
